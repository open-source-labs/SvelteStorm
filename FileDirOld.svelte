      ...
      *     o  One for importing path
       ...
       function updateRollupConfig (path) {
        ...
        if (file.split('.').pop() === 'svelte'){
          ...
          // svelteFileArray.push(file.split('.')[0])
          
          
          if(path.includes('node_modules') !== true) {
            svelteFileArray.push(`${path}/${file}`)
...
Start with App.svelte file. Open it and look for children via 'import' statement.

Create ParentChildTree empty object

Create list for children

ParentChildTree = findChildren(App.svelte)



findChildren (FileToProcess) {
  
  // Process FileToProcess (separate function) => Returns array of children


  // Exit case = No children in opened file.
    if (!childFoundCount)  retun {}
      
  for(let i = 0; i < childFound; i++) {

    // For each child ... open child and look for children
    
    return {thisChild[i]: findChildren( thisChild )}
  }
    ...
    // If children ... add children to some list
    // findChildren( thisChild ) // Then recurse those children
    // return {}
...
/*
reorder array of files so app is first
start with app, create parent child tree
look at children of App in parent child tree
  for each, create parent fild tree 
    look at children of each, apply same function

Build parent child tree object with App = AppTree via below
function buildParentChildTreeHelper (relationships) {
  If Object.keys(relationships).length > 0
    for each element of relationships
    access file within array of files
    build out relationship Tree
    add newtree to relationships
    invoke buildParentChildTreeHelper on newTree
} else return

buildParentChildTreeHelper(App)
...
*/
  ...
  for(let i = 0; i < svelteFileArray.length; i++) {

    var content:string = fs.readFileSync(svelteFileArray[i]).toString();                    
    /*
    * ==================================================
    ...
    */
    ...
    console.log('🔴🟠🟡🟢🔵🟣 | file: FileDir.svelte | line 441 | buildParentChildTree | fileName', fileName);

    tParentChildTree[fileParts[0]] = {}

    for(let i = 0; i < importsArray.length; i++) {
      let foundComp = (importsArray[i].split(' '));
      let reallyFoundComp = foundComp[1];
      tParentChildTree[fileParts[0]][reallyFoundComp] = {};
    }
...
/*
* ==================================================
*   Access current value in store
*   Make copy of ParentChildTree from store
*
*   
* ==================================================
*/
